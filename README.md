...
а начало года — отличное время для того чтобы узнать,
...
В некоторых языках есть договорённость, называть приватные методы через видимый пробел
<sup>( «**_**» — такая_штука, ты можешь знать этот знак под неверным названием — нижнее подчёркивание)</sup>
...
<sup>Лично мне импонирует идея использовать видмый пробел в качестве префикса для функций, возвращающих `this`.
Так их можно объединять в цепочку вызовов.</sup>
...
Но в целом JS радует глаз своей лаконичностью по сравнению с PHP вариантом.
...
Синтаксически, это выглядит как вызов функции `import()`,
но не наследуется от Function.prototype,
а значит вызвать через call или apply — не удастся.
